...
| 長押し（0.7秒） | **キャンセル**。画面が白くなり「キャンセル」。画面の下端に「長押しでキャンセル」と書いてある |
| 終わった／キャンセルした画面をタップ | 新しく始める |
| 上の中央の「⏱ 1:30」 | 時間を変える。±10秒・±1分の4つと Digital Crown（1目盛り10秒）。「完了して戻る」で保存して最初から。やめるならクラウンを押す |
| 設定の「色 n」 | 押すたびに 1→…→10→1。水・終わった画面・ボタン・文字盤のコンプリケーションの色が変わる |
| 終わった | 触覚を**一度だけ**（`.notification`×3 → `.success`）。画面が白く抜けて「おわり」。タップで最初から |
| 腕を下ろしていた | 終わる時刻に通知が1回（触覚つき）。前に出ている間は通知を出さず、自分で鳴らす |
| 走っている最中に開き直した | 続き。**終わってから30秒以内**に開いたら「おわり」のまま、それ以降なら新しく始まる |
